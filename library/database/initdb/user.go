...
type User struct {
	...
	Nickname  string    `gorm:"column:nickname;type:varchar(120);comment:昵称"`
	Password  string    `gorm:"column:password;type:varchar(255);comment:密码"`
	Sex       int8      `gorm:"column:sex;size:1;not null;default:0;comment:性别: 0保密 1男 2女"`
	Birthday  time.Time `gorm:"column:birthday;type:varchar(60);comment:生日"`
	Mobile    string    `gorm:"column:mobile;type:varchar(20);uniqueIndex;comment:手机号"`
	Email     string    `gorm:"column:email;type:varchar(20);uniqueIndex;comment:邮箱"`
	...
	Degree    int8      `gorm:"column:degree;size:1;not null;default:0;comment:学位: 1学士 2硕士 3博士"`
	Usertype  int8      `gorm:"column:usertype;size:1;not null;default:0;comment:用户类型: 1普通用户 2企业用户 3管理员"`
	Status    int8      `gorm:"column:status;size:1;not null;default:1;comment:用户类型: 1正常 2拉黑"`
...
type UserBalanceLog struct {
	GormModel
	UserID        uint      `gorm:"column:user_id;not null;comment:用户ID"`
	Action        int8      `gorm:"column:action;size:1;not null;default:0;comment:行为: 1充值 2消费 3推广 4提现 5岗位扣费"`
...
type UserCertification struct {
	...
	Industry        uint   `gorm:"column:industry;not null;comment:所属行业"`
	CompanyName     string `gorm:"column:company_name;type:varchar(90);uniqueIndex;comment:公司名称"`
	...
	Realname        string `gorm:"column:realname;type:varchar(30);comment:真实姓名"`
	...
	Status          int8   `gorm:"column:status;size:1;default:0;comment:状态: 0待审核 1通过 2失败"`
