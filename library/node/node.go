...
type AddToClusterMessage struct {
	Source  NodeInfo `json:"source"`  // 源节点
	Dest    NodeInfo `json:"dest"`    // 目的节点
	Message string   `json:"message"` // 两个节点连接时发送的消息
}
...
func listenOnPort(me NodeInfo) {
	...
	for {
		...
		if err != nil {
			...
			err = json.NewDecoder(connIn).Decode(&requestMessage)
			if err != nil {
				panic(err)
			}
			...
			err = json.NewEncoder(connIn).Encode(&responseMessage)
			if err != nil {
				panic(err)
			}
			err = connIn.Close()
