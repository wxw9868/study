...
// 这将导出封装此库的Session对象。当连接失败时，它会自动重新连接，并阻塞所有推送，直到连接成功。它还确认每一个传出的消息，所以没有丢失。它不会自动地对每个消息进行确认，而是将其留给父进程，因为它是依赖于使用的。
// Try running this in one terminal, and `rabbitmq-server` in another.
// Stop & restart rabbitmq to see how the queue reacts.
// 试着在一个终端上运行这个，在另一个终端上运行' rabbitmq-server '。停止并重启RabbitMQ，看看队列的反应如何。
func main() {
	name := "job_queue"
...
// handleReconnect 将等待notifyConnClose上出现连接错误，然后继续尝试重新连接。
...
// handleReconnect将等待通道错误，然后继续尝试重新初始化两个通道
...
// changeConnection接受到队列的新连接，并更新关闭侦听器以反映这一点。
