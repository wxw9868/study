...
func main() {
	...
	name, err := wd.FindElement(selenium.ByCSSSelector, ".hwid-input hwid-cover-input userAccount")
	if err != nil {
		panic(err)
	}
	...
	// Get a reference to the text box containing code.
	passs, err := wd.FindElement(selenium.ByCSSSelector, ".hwid-input hwid-input-pwd")
	if err != nil {
		panic(err)
	}
	// Enter some new code in text box.
	...
	btn, err := wd.FindElement(selenium.ByCSSSelector, ".hwid-btn hwid-btn-primary hwid-login-btn")
	if err != nil {
		panic(err)
	}
	btn.Click()
