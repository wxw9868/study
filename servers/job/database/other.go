...
type Region struct {
	...
	Code     string `gorm:"column:code;type:varchar(20);uniqueIndex;not null;comment:地区编码"`
	PostCode string `gorm:"column:post_code;type:varchar(20);index;comment:邮编"`  // Added index
	Level    int8   `gorm:"column:level;type:tinyint(1);default:0;comment:地区级别"` // Added index
}
